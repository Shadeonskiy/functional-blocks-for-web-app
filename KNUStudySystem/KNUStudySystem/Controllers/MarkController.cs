        ...
        private List<Mark> ParseDB(string filters)
        {
            ...
            using (var connection = new MySqlConnection(_database.Connection))
            {
                ...
                _database.setCommand("SELECT tasks.task_id, tasks.task_name, tasks.subject, " +
                    "tasks.task_type, marks_test.id, marks_test.comment, marks_test.grade,  " +
                    "marks_test.teacher, marks_test.evaluation_date, marks_test.status " +
                    "FROM tasks JOIN marks_test ON tasks.task_id = marks_test.task_id " +
